...
fs.readFile('snalist.csv', 'utf8', async (err, data) => {
  ...
  for (let domain of sna_domains) {
    if (!sna_seen.has(domain)) { // If the domain is not in the set
  ...
  if (!process.env.CI) console.log(`Found ${sna_domains.length} valid domains in snalist.csv after cleanup - ${sna_listsToCreate} list(s) will be created`);
  ...
  for (const [index, chunk] of sna_chunks.entries()) {
    ...
    try {
      await createZeroTrustList(sna_listName, properList, (index+1), sna_listsToCreate);
...
fs.readFile('input.csv', 'utf8', async (err, data) => {
  ...
  // Trim array to 300,000 domains if it's longer than that
  const left_LIST_ITEM_LIMIT = LIST_ITEM_LIMIT - sna_domains.length
