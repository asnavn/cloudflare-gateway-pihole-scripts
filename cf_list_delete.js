...
;(async() => {
    ...
    if (!sna_lists.length) return console.warn("No lists with matching name found - this is not an issue if you haven't created any filter lists before. Exiting.");

    if (!process.env.CI) console.log(`Got ${lists.length} lists, ${sna_lists.length} of which are SNA lists that will be deleted.`);
