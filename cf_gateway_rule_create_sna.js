...
(async () => {
  ...
  const filtered_lists = lists.filter((list) =>
    list.name.startsWith("SNA List")
  );
  ...
  if (!process.env.CI)
    console.log(
      `Firewall expression contains ${wirefilter_expression.length} characters, and checks against ${filtered_lists.length} filter lists.`
    );
